 ...
 *   1. 七个快捷键行各自绑到正确的 settings 字段（类型全是 string，接错不报错）
...
function renderTab(overrides: Partial<Parameters<typeof HotkeysTab>[0]> = {}) {
  const props = {
    settings: makeSettings({
      ...
      screenshotAnnotate: { hotkey: 'Alt+6' },
      lens: { hotkey: 'Alt+7' },
    } as never),
    t,
...
describe('HotkeysTab', () => {
  it('每个快捷键行显示各自 settings 字段的值（不串行）', () => {
    ...
    // 七个值的数字互不相同，任一 props 接错都会让某个数字缺失或重复。
  ...
  it('渲染全部七个快捷键行', () => {
    renderTab()
    // 每行一个「录制」按钮
    expect(screen.getAllByRole('button', { name: t.hotkeyRecord })).toHaveLength(7)
  })
  ...
  it('录制按钮把对应 scope 传给 onToggleRecording', async () => {
    ...
    // 第 3-5 个是截图翻译系列，第 6 个标注，第 7 个 Lens
