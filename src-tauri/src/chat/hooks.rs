...
async fn run_job(shared: &Shared, job: HookJob) {
    ...
    for hook in hooks {
        ...
        let result = match hook.kind.as_str() {
            "http" => run_http_hook(hook, &body).await,
...
mod tests {
    ...
    async fn script_receives_stdin_json_and_env() {
        ...
        let hook = command_hook(&format!(
            "cat > {out}; printf '%s|%s|%s|%s' \"$KIVIO_HOOK_EVENT\" \"$KIVIO_HOOK_NAME\" \"$KIVIO_TOOL_NAME\" \"$KIVIO_CONVERSATION_ID\" > {env}",
            out = out.display(),
            env = env_out.display()
        ));
        let (dispatcher, failures) =
            dispatcher(&[hook], dir.path().to_path_buf()).expect("dispatcher");
        dispatcher.dispatch(HookEvent::AgentEnd, Some("write_file"), Some(2));
        ...
        assert_eq!(
            ...
            "agent_end|probe|write_file|conv-1"
        );
        assert!(
    ...
    async fn cancel_drops_queued_hooks_and_kills_the_running_script() {
        ...
        dispatcher.dispatch(HookEvent::AgentEnd, None, None);
        dispatcher.dispatch(HookEvent::TurnEnd, None, None);
