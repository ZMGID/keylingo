...
29. **stdout 上的帧必须按类型分流，而「在等回复的 `control_request`」不许沉默**（07-29，全部形状从本机 claude 2.1.220 二进制 `grep -a` 核实 + 真机对照）。判据是纯函数 `session/claude_stream.rs::classify_inbound_frame`。
   ...
   - **`control_cancel_request` 有意不接**：它是 CLI 撤回自己先前发出的某个 `control_request`（权限询问已在别处答掉）。CLI 自己收到这条时也只 `abort()` 在飞的请求、**不回任何东西**，所以静默忽略是对的；实现权限问答时这里要改成「取消那条待答的询问」。`control_response`（我们那条 `interrupt` 的 ack）同样有意不接：取消的权威信号是那一轮的 `result`，ack 可能在我们已经返回之后才到，读它没有决策价值。
