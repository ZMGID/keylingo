...
**Binding contracts live in `.trellis/spec/`, not here.** This file is the map; the spec dir holds the enforceable details. Start from `.trellis/spec/guides/index.md` and `.trellis/spec/frontend/index.md`, and read the relevant doc before editing that area — notably `guides/external-cli-agents.md` (required before touching `external_agents/`), `guides/subagent-role-definitions.md` (required before touching `agents/` or sub-agent tool narrowing), `frontend/builtin-web-search.md`, `frontend/state-management.md`, `frontend/resource-lifecycle.md`, and `frontend/component-guidelines.md`.
